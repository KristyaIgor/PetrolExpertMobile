...
import md.intelectsoft.petrolexpert.emvcardreader.iso7816emv.impl.TagImpl;
...
public final class EmvTags {
	...
	//IntelectSoft data objects for application and selection
	public static final ITag VEHICLE_TAG_CARD_DKV = new TagImpl("df42", TagValueTypeEnum.BINARY, "Vehicle tag on the card", "IN this tag set vehicle5");
	public static final ITag Fuel_card_usage_bitmap_DKV = new TagImpl("df30", TagValueTypeEnum.TEXT, "Fuel card usage bitmap", "Bitmap fuel car usage");
	...
	public static ITag find(final byte[] tagBytes) {
		ByteArrayWrapper arrayWrapper = ByteArrayWrapper.wrapperAround(tagBytes);
		ITag obtainedTag = tags.get(arrayWrapper);
		return obtainedTag;
	}
