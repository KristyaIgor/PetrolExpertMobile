...
import md.intelectsoft.petrolexpert.emvcardreader.exception.TlvException;
...
//		String indent = getSpaces(indentLength);
		...
		while (stream.available() > 0) {
			...
			if(tag.getName().equals("Application Label"))
				Log.e("PetrolExpert_BaseApp", "getFormattedTagAndLength: " +  prettyPrintHex(tag.getTagBytes()));
	...
	public static List<TLV> getlistTLV(final byte[] pData, final ITag... pTag) {
		...
		while (stream.available() > 0) {
			...
			if (TlvContains(tlv.getTag(),pTag)) {
				list.add(tlv);
			} else if (tlv.getTag().isConstructed()) {
	...
	public static boolean TlvContains(ITag tag, ITag... listTags) {
		return indexOf(tag, listTags) != -1;
	}
	public static int indexOf(ITag tag, ITag... listTags) {
		return indexOf(tag, 0, listTags);
	}

	public static int indexOf(ITag tag, int startIndex, ITag... listTags) {
		if (listTags == null) {
			return -1;
		} else {
			if (startIndex < 0) {
			...
			int i;
			if (tag == null) {
				for(i = startIndex; i < listTags.length; ++i) {
					if (listTags[i] == null) {
						return i;
					}
				}
			} else if (listTags.getClass().getComponentType().isInstance(tag)) {
				for(i = startIndex; i < listTags.length; ++i) {
					if (listTags[i].getName().equals(tag.getName())) {
						return i;
					}
				}
			}
	...
	public static byte[] getValue(final byte[] pData, final ITag... pTag) {
		...
		if (pData != null) {
			...
			while (stream.available() > 0) {
				...
				TLV tlv = TlvUtil.getNextTLV(stream);
				if(pTag.length > 1 ){
					if (pTag[0].getName().equals(tlv.getTag().getName()) || pTag[1].getName().equals(tlv.getTag().getName())) {
					...
					if (pTag[0].getName().equals(tlv.getTag().getName())) {
						return tlv.getValueBytes();
					} else if (tlv.getTag().isConstructed()) {
						ret = TlvUtil.getValue(tlv.getValueBytes(), pTag);
						if (ret != null) {
...
//					buf.append(getSpaces(0));
			...
			if(tag.getName().equals("Application Label"))
				Log.e("PetrolExpert_BaseApp", "getTagValueAsString: " + TlvUtil.getTagValueAsString(tag, valueBytes));
			...
			if (tag.isConstructed()) {
				// indentLength += extraIndent; //TODO check thislo
				// Recursion
